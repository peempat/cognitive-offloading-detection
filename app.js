...
const COURSES = {
  coding: {
    en: "Coding",
    th: "โค้ดดิ้ง",
    promptEn: "Example: I need help finding the bug in this loop.",
    promptTh: "ตัวอย่าง: ช่วยดูวิธีคิดโค้ดวนซ้ำนี้ให้หน่อย",
  },
  english: {
    en: "English",
    th: "ภาษาอังกฤษ",
    promptEn: "Example: Help me understand this sentence without translating it all.",
    promptTh: "ตัวอย่าง: ช่วยแยกประโยคภาษาอังกฤษนี้ให้เข้าใจหน่อย",
  },
  math: {
    en: "Math",
    th: "คณิตศาสตร์",
    promptEn: "Example: I need help setting up this math problem: 24 divided by 6.",
    promptTh: "ตัวอย่าง: ช่วยตั้งวิธีคิดโจทย์ 24 หาร 6 ให้หน่อย",
  },
  science: {
    en: "Science",
    th: "วิทยาศาสตร์",
    promptEn: "Example: Help me understand why plants need light.",
    promptTh: "ตัวอย่าง: ช่วยอธิบายว่าทำไมพืชต้องใช้แสง",
  },
  social: {
    en: "Social Studies",
    th: "สังคมศึกษา",
    promptEn: "Example: Help me make a timeline for this history lesson.",
    promptTh: "ตัวอย่าง: ช่วยเรียงเหตุการณ์บทเรียนประวัติศาสตร์นี้",
  },
  thai: {
    en: "Thai",
    th: "ภาษาไทย",
    promptEn: "Example: Help me find the main idea of this Thai paragraph.",
    promptTh: "ตัวอย่าง: ช่วยหาประเด็นสำคัญของย่อหน้านี้",
  },
};

const DEFAULT_BEHAVIOR_LANG = "th";
const LOG_STORAGE_KEY = "kamunLogs";
const LEGACY_LOG_STORAGE_KEY = "focusBuddyLogs";
const BEHAVIOR_COPY = {
  en: {
    analyze: "Analyze",
...
const els = {
  ...
  languageButtons: [],
  logMenu: $("#logMenu"),
  logMenuBtn: $("#logMenuBtn"),
  logPreviewList: $("#logPreviewList"),
...
function selectedCourse() {
  return COURSES[state.selectedCourse] || COURSES.math;
}

function selectedCourseName(language = state.behaviorLanguage) {
  const course = selectedCourse();
  return language === "th" ? course.th : course.en;
}

function courseHintLine(courseId = state.selectedCourse, language = state.behaviorLanguage) {
  const hints = {
    coding: {
      en: "For coding, describe what you expected, what happened, and the smallest line you want to test first.",
      th: "ถ้าเป็นโค้ดดิ้ง ให้บอกก่อนว่าอยากให้โค้ดทำอะไร ตอนนี้เกิดอะไรขึ้น และอยากลองตรวจบรรทัดไหนก่อน",
    },
    english: {
      en: "For English, circle the words you know first, then guess the job of one unknown word from the sentence.",
      th: "ถ้าเป็นภาษาอังกฤษ ให้หาคำที่รู้ก่อน แล้วลองเดาหน้าที่ของคำที่ยังไม่รู้จากประโยครอบ ๆ",
    },
    math: {
      en: "For math, write one line of what you know, then choose whether the next step is add, subtract, multiply, or divide and why.",
      th: "ถ้าเป็นคณิต ให้เขียนสิ่งที่รู้ 1 บรรทัด แล้วเลือกว่าจะบวก ลบ คูณ หรือหารเพราะอะไร",
    },
    science: {
      en: "For science, name the thing that changes and the thing you are observing, then explain the pattern in your own words.",
      th: "ถ้าเป็นวิทยาศาสตร์ ให้แยกก่อนว่าอะไรเปลี่ยน อะไรที่เราสังเกต แล้วลองอธิบายความสัมพันธ์ด้วยคำของตัวเอง",
    },
    social: {
      en: "For social studies, place the event, person, or place on a simple who-what-when-why map.",
      th: "ถ้าเป็นสังคมศึกษา ให้จัดข้อมูลเป็น ใคร ทำอะไร เมื่อไร ที่ไหน และเพราะอะไร ก่อน",
    },
    thai: {
      en: "For Thai, find the main idea first, then underline one sentence that supports it.",
      th: "ถ้าเป็นภาษาไทย ให้หาใจความสำคัญก่อน แล้วขีดเส้นประโยคที่ช่วยสนับสนุนใจความนั้น",
    },
  };
  return (hints[courseId] || hints.math)[language === "th" ? "th" : "en"];
}
...
function looksLikeCalculation(text) {
  ...
  const hasCalculationWord =
    /(บวก|ลบ|คูณ|หาร|เท่ากับ|คิดเลข|คำนวณ|สมการ|เศษส่วน|เปอร์เซ็นต์|ร้อยละ|พื้นที่|ปริมาตร|add|subtract|multiply|divide|calculate|equation|fraction|percent|percentage|area|volume)/i.test(
      normalized,
    );
  return (hasDigits && (hasOperator || hasCalculationWord)) || hasCalculationWord;
}

function wantsBriefSummary(text) {
  return /(สรุป|ใจความสำคัญ|สั้นๆ|สั้น ๆ|ประเด็นสำคัญ|นิยาม|คืออะไร|summary|main idea|key point|brief|definition|what is)/i.test(
    text,
  );
}
...
function exportSession() {
  ...
  anchor.href = url;
  anchor.download = `kamun-session-${new Date().toISOString().slice(0, 19).replaceAll(":", "-")}.json`;
  anchor.click();
...
function addEvent(event) {
  ...
  renderEvents();
  persistSessionLog("event", { event: state.events[0] });
}

function persistSessionLog(type, data = {}) {
  const entry = {
    at: new Date().toISOString(),
    focus: Math.round(state.focus || 0),
    latestMetrics: state.lastMetrics
      ? {
  ...
  try {
    ...
    previous.push(entry);
    localStorage.setItem(LOG_STORAGE_KEY, JSON.stringify(previous.slice(-500)));
  } catch (error) {
    console.warn("Unable to save local focus log", error);
  }
  sendBackendLog(entry);
}

async function sendBackendLog(entry) {
  try {
    ...
    console.warn("Unable to save backend focus log", error);
  }
}

function maybeLogCameraMetrics(now) {
  if (!state.lastMetrics || now - state.lastBackendLogAt < 5000) return;
  state.lastBackendLogAt = now;
  persistSessionLog("camera-snapshot", {
    raw: state.lastMetrics.raw,
  });
}
...
async function loadBackendLogs() {
  if (!els.logPreviewList) return;
  els.logPreviewList.innerHTML = "<li>กำลังโหลด log...</li>";
  try {
    const response = await fetch("/api/focus-logs");
    const payload = await response.json();
    renderBackendLogs(payload.logs || []);
  } catch (error) {
    console.warn("Unable to load focus logs", error);
    els.logPreviewList.innerHTML = "<li>โหลด log ไม่สำเร็จ</li>";
  }
}
...
function renderBackendLogs(logs) {
  ...
  if (!latest.length) {
    els.logPreviewList.innerHTML = "<li>ยังไม่มี log</li>";
    return;
  }
  els.logPreviewList.innerHTML = latest
    .map((entry) => {
      const time = entry.at ? new Date(entry.at).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" }) : "--:--";
      const focus = Number.isFinite(entry.focus) ? `${entry.focus}/100` : "ไม่มีคะแนน";
      const type = entry.type === "camera-snapshot" ? "กล้อง" : entry.type === "checkin" ? "LLM" : "เหตุการณ์";
      return `<li><strong>${time} · ${type}</strong><span>${focus}</span></li>`;
    })
    .join("");
}

function toggleLogMenu(forceOpen) {
  if (!els.logMenu || !els.logMenuBtn) return;
  const open = typeof forceOpen === "boolean" ? forceOpen : els.logMenu.hidden;
  els.logMenu.hidden = !open;
  els.logMenuBtn.setAttribute("aria-expanded", String(open));
  if (open) loadBackendLogs();
}
...
function assessBehavior(text, metrics) {
  ...
  const assistantText = buildHomeworkCoachResponse(text, metrics, signal, flags);
  return { assistantText, flags, signal, signalScore };
}

function buildHomeworkCoachResponse(text, metrics, signal, flags) {
  const thai = state.behaviorLanguage === "th";
  const courseName = selectedCourseName();
  const trimmed = text.trim();
  const guidedOnly = looksLikeCalculation(trimmed) || state.selectedCourse === "coding";
  const briefSummary = wantsBriefSummary(trimmed);
  if (thai) {
    if (!guidedOnly) {
      return [
        `กมลเห็นคำถามวิชา${courseName}แล้ว: "${trimmed}"`,
        briefSummary
          ? "สรุปสั้น ๆ คือ ให้จับใจความหลักก่อน แล้วจำเฉพาะคำสำคัญที่อธิบายเรื่องนี้"
          : "คำถามนี้กมลตอบให้ตรง ๆ ได้ พร้อมอธิบายสั้น ๆ ให้อ่านง่าย",
        "ถ้าคำตอบยังไม่พอดี ลองส่งรายละเอียดโจทย์เพิ่มอีกนิด เช่น ข้อความเต็ม รูปแบบคำถาม หรือส่วนที่ยังงง",
        courseHintLine(),
      ].join(" ");
    }
    return [
      `กมลเห็นโจทย์${courseName}ของเราแล้ว: "${trimmed}"`,
      looksLikeCalculation(trimmed)
        ? "ถ้าเป็นการคิดเลข กมลจะยังไม่เฉลยคำตอบสุดท้ายทันทีนะ เราจะค่อย ๆ คิดทีละขั้น"
        : "วิชานี้กมลจะพาคิดทีละขั้นก่อน ยังไม่ให้คำตอบเต็มทันที",
      "ลองบอกก่อนว่าโจทย์ถามหาอะไร และมีข้อมูลสำคัญคำไหนบ้าง",
      courseHintLine(),
    ].join(" ");
  }
  if (!guidedOnly) {
    return [
      `I see your ${courseName} question: "${trimmed}"`,
      briefSummary
        ? "Briefly, focus on the main idea first and remember only the key words that explain it."
        : "This is not a calculation, so I can answer directly and keep the explanation short and easy to read.",
      "If the answer needs more detail, send the full question or the part that feels confusing.",
      courseHintLine(),
    ].join(" ");
  }
  return [
    `I see your ${courseName} homework question: "${trimmed}"`,
    looksLikeCalculation(trimmed)
      ? "Because this is a calculation, I will not give the final answer right away. We will work step by step."
      : "For this subject, I will guide you step by step before giving a full solution.",
    "First, tell me what the question is asking and which details look important.",
    courseHintLine(),
  ].join(" ");
}
...
function legacyScoreTone(score) {
  const copy = appCopy();
...
function legacyAnalyzeFace(result, now) {
  const copy = signalCopy();
  const landmarks = result.faceLandmarks?.[0];
  if (!landmarks) {
    const staleMs = state.lastFaceSeen ? now - state.lastFaceSeen : 9999;
...
function legacyAnalyzePosture(result, face) {
  const copy = signalCopy();
  const pose = result.landmarks?.[0];
  if (!pose) {
    return {
...
function legacyAnalyzeActions(result, face) {
  const copy = signalCopy();
  const hands = result.landmarks || [];
  const centers = hands.map((hand) => pointAverage(hand)).filter(Boolean);
  const nearFace = Boolean(
...
function legacyUpdateUI(metrics) {
  const focus = Math.round(metrics.focus || 0);
  const tone = scoreTone(focus);
  const copy = appCopy();
  els.focusScore.textContent = String(focus);
...
async function legacyStartCamera() {
  if (!state.models || state.active) return;
  try {
    state.stream = await navigator.mediaDevices.getUserMedia({
      audio: false,
...
async function legacyLoadModels() {
...
function applyBehaviorLanguage() {
  ...
  updateBehaviorUI();
  applyStaticLanguage();
  applyCourseSelection(state.selectedCourse);
}
...
function applyCourseSelection(courseId) {
  if (!COURSES[courseId]) return;
  state.selectedCourse = courseId;
  const course = selectedCourse();
  els.courseButtons.forEach((button) => {
    button.classList.toggle("active", button.dataset.course === courseId);
  });
  if (els.selectedCourseLabel) {
    els.selectedCourseLabel.textContent = selectedCourseName();
  }
  if (els.chatInput) {
    els.chatInput.placeholder = state.behaviorLanguage === "th" ? course.promptTh : course.promptEn;
  }
}
...
function bindEvents() {
  ...
  els.logMenuBtn?.addEventListener("click", () => toggleLogMenu());
  els.refreshLogsBtn?.addEventListener("click", loadBackendLogs);
  document.addEventListener("click", (event) => {
    if (!els.logMenu || !els.logMenuBtn) return;
    if (els.logMenu.hidden) return;
    if (els.logMenu.contains(event.target) || els.logMenuBtn.contains(event.target)) return;
    toggleLogMenu(false);
  });
  document.querySelectorAll(".segment").forEach((button) => {
  ...
  els.courseButtons.forEach((button) => {
    button.addEventListener("click", () => applyCourseSelection(button.dataset.course));
  });
...
Object.assign(BEHAVIOR_COPY.en, {
  analyze: "Help me learn",
  assistantName: "kamun",
  behaviorSignal: "Learning clue",
  chatLabel: "Ask kamun",
  clear: "Clear",
  disclaimer:
    "kamun gives step-by-step hints for Math and Coding. For other subjects, it can answer directly in kid-friendly language.",
  editRatio: "Oops fixes",
  idle: "Ready",
  intro: "Hi! Pick a subject and ask your question. Math and Coding use step-by-step hints; other subjects can get direct answers.",
  messageLength: "Message size",
  placeholder: "Example: I need help setting up this math problem: 24 divided by 6.",
  recordingWindow: "Thinking time",
  reviewing: "Thinking",
  signalLabels: { Elevated: "A bit wiggly", High: "Very wiggly", Low: "Calm", Moderate: "Some wiggles" },
  startedAt: "Started",
  title: "kamun homework",
  typing: "Typing",
  typingSpeed: "Typing pace",
  userName: "Me",
});

Object.assign(BEHAVIOR_COPY.th, {
  analyze: "ช่วยเรียนรู้",
  assistantName: "กมล",
  behaviorSignal: "คำใบ้การเรียน",
  chatLabel: "ถามกมล",
  chars: "ตัวอักษร",
  clear: "ล้าง",
  disclaimer:
    "กมลจะพาคิดทีละขั้นในคณิตศาสตร์และโค้ดดิ้ง ส่วนวิชาอื่นถามแล้วตอบตรงได้ด้วยภาษาที่เด็กอ่านง่าย",
  editRatio: "แก้คำ",
  idle: "พร้อม",
  intro: "สวัสดี! เลือกวิชาแล้วถามได้เลย คณิตศาสตร์กับโค้ดดิ้งกมลจะพาคิดทีละขั้น ส่วนวิชาอื่นตอบตรงได้",
  messageLength: "ขนาดข้อความ",
  placeholder: "ตัวอย่าง: ช่วยตั้งวิธีคิดโจทย์ 24 หาร 6 ให้หน่อย",
  recordingWindow: "เวลาคิด",
  reviewing: "กำลังคิด",
  signalLabels: { Elevated: "วอกแวกขึ้น", High: "วอกแวกมาก", Low: "นิ่งดี", Moderate: "วอกแวกนิดหน่อย" },
  startedAt: "เริ่ม",
  title: "กมลช่วยการบ้าน",
  typing: "กำลังพิมพ์",
  typingSpeed: "จังหวะพิมพ์",
  userName: "ฉัน",
});

Object.assign(APP_COPY.en, {
  action: "Hands",
  actionWaiting: "Hands: waiting",
  actionTolerance: "Hand wiggle level",
  calibrate: "Set my seat",
  calibrationHint: "Sit comfy, look at the screen, then save this as your starting pose.",
  calibrationNotReady: "Seat not saved yet",
  cameraEmptyHint: "Press Start. Keep your face in the picture.",
  cameraEmptyTitle: "Ready?",
  calibrated: "Seat saved",
  cameraBlocked: "Camera needs help",
  cameraPermissionHint: "Please allow camera permission on localhost.",
  currentScore: "kamun score",
  events: "kamun notes",
  export: "Save",
  eye: "Eyes",
  eyeWaiting: "Eyes: waiting",
  eyeTolerance: "Looking-away level",
  focus: "Stars",
  focusHintIdle: "Press Start and do one small task.",
  focusLabelIdle: "Ready",
  liveSignals: "Press Start. Look at your work.",
  mode: "Game mode",
  modelError: "kamun could not wake up",
  modelNetworkHint: "Check internet and refresh this page.",
  modelNotReady: "kamun is not ready yet",
  noEvents: "No kamun notes yet",
  posture: "Sitting",
  postureTolerance: "Slouch level",
  postureWaiting: "Sitting: waiting",
  presence: "On screen",
  reset: "Restart",
  scoreGood: "Great job",
  scoreGoodHint: "Keep going.",
  scoreLow: "Try again",
  scoreLowHint: "Take one breath. Pick one tiny step.",
  scoreWarn: "Look back",
  scoreWarnHint: "Eyes on your task.",
  start: "Start logging",
  statusLoading: "Waking kamun",
  statusMeasuring: "kamun watching",
  statusPartial: "kamun partly ready",
  statusReady: "kamun ready",
  stop: "Pause log",
  trend: "Star path",
  waiting: "waiting",
});

Object.assign(APP_COPY.th, {
  action: "มือ",
  actionWaiting: "มือ: รอข้อมูล",
  actionTolerance: "ระดับมือขยับ",
  calibrate: "บันทึกท่านั่ง",
  calibrationHint: "นั่งสบาย ๆ มองหน้าจอ แล้วบันทึกเป็นท่าเริ่มต้นของเรา",
  calibrationNotReady: "ยังไม่ได้บันทึกท่านั่ง",
  cameraEmptyHint: "กดเริ่ม แล้วให้เห็นหน้าเราในภาพ",
  cameraEmptyTitle: "พร้อมไหม",
  calibrated: "บันทึกท่านั่งแล้ว",
  cameraBlocked: "กล้องยังเปิดไม่ได้",
  cameraPermissionHint: "กดอนุญาตใช้กล้องบน localhost ก่อนนะ",
  currentScore: "คะแนนกมล",
  events: "โน้ตจากกมล",
  export: "บันทึก",
  eye: "ตา",
  eyeWaiting: "ตา: รอข้อมูล",
  eyeTolerance: "ระดับมองออกนอกจอ",
  focus: "ดาว",
  focusHintIdle: "กดเริ่ม แล้วทำงานเล็ก ๆ หนึ่งอย่าง",
  focusLabelIdle: "พร้อมแล้ว",
  liveSignals: "กดเริ่ม แล้วมองงานของเรา",
  mode: "โหมดเกม",
  modelError: "กมลตื่นไม่สำเร็จ",
  modelNetworkHint: "เช็กอินเทอร์เน็ตแล้วรีเฟรชหน้านี้อีกครั้ง",
  modelNotReady: "กมลยังไม่พร้อม",
  noEvents: "ยังไม่มีโน้ตจากกมล",
  posture: "ท่านั่ง",
  postureTolerance: "ระดับนั่งย้วย",
  postureWaiting: "ท่านั่ง: รอข้อมูล",
  presence: "อยู่หน้าจอ",
  reset: "เริ่มใหม่",
  scoreGood: "เก่งมาก",
  scoreGoodHint: "ทำต่อได้เลย",
  scoreLow: "ลองใหม่",
  scoreLowHint: "หายใจหนึ่งครั้ง แล้วเลือกงานเล็ก ๆ หนึ่งอย่าง",
  scoreWarn: "กลับมามองงาน",
  scoreWarnHint: "มองงานของเรานะ",
  start: "เริ่มบันทึก",
  statusLoading: "ปลุกกมล",
  statusMeasuring: "กมลกำลังดู",
  statusPartial: "กมลพร้อมบางส่วน",
  statusReady: "กมลพร้อมแล้ว",
  stop: "พักบันทึก",
  trend: "เส้นทางดาว",
  waiting: "รอข้อมูล",
});

Object.assign(SIGNAL_COPY.en, {
  calibratedEvent: "Seat saved",
  eyesClosed: "Eyes resting",
  eyesClosedEvent: "Eyes rested for a while",
  fidget: "Hands got busy",
  fidgetLabel: "Busy hands",
  handFace: "Hand near face",
  handOptional: "Hand clues are quiet",
  lookingAtScreen: "Eyes on task",
  lookingAway: "Looking away",
  lookAwayEvent: "Eyes left the task",
  lowFocusEvent: "kamun score dipped",
  noFace: "kamun cannot see you",
  postureBad: "Sitting got floppy",
  postureGood: "Sitting steady",
  posturePartial: "kamun cannot see your sitting pose",
  postureEvent: "Sitting pose changed",
  recovered: "Back on track",
  still: "Calm",
  waiting: "waiting",
});

Object.assign(SIGNAL_COPY.th, {
  calibratedEvent: "บันทึกท่านั่งแล้ว",
  eyesClosed: "พักสายตา",
  eyesClosedEvent: "พักสายตานานไปนิด",
  fidget: "มือขยับเยอะ",
  fidgetLabel: "มือยุ่งนิดหน่อย",
  handFace: "มือใกล้หน้า",
  handOptional: "สัญญาณมือเงียบดี",
  lookingAtScreen: "ตาอยู่กับงาน",
  lookingAway: "มองออกนอกจอ",
  lookAwayEvent: "ตาออกจากงาน",
  lowFocusEvent: "คะแนนกมลลดลง",
  noFace: "กมลมองไม่เห็นเรา",
  postureBad: "ท่านั่งเริ่มย้วย",
  postureGood: "ท่านั่งนิ่งดี",
  posturePartial: "กมลยังเห็นท่านั่งไม่ชัด",
  postureEvent: "ท่านั่งเปลี่ยน",
  recovered: "กลับมาเข้าที่แล้ว",
  still: "นิ่งดี",
  waiting: "รอข้อมูล",
});

scoreTone = legacyScoreTone;
analyzeFace = legacyAnalyzeFace;
analyzePosture = legacyAnalyzePosture;
analyzeActions = legacyAnalyzeActions;
updateUI = legacyUpdateUI;
startCamera = legacyStartCamera;
stopCamera = legacyStopCamera;
resetSession = legacyResetSession;
calibrate = legacyCalibrate;
loadModels = legacyLoadModels;
